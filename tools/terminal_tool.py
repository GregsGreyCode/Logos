...
def terminal_tool(
    ...
    try:
        ...
        # Enforce workspace terminal policy (dry_run blocking, workspace CWD
        # injection) when a workspace is configured for this task.
        #
        # Trust model: these are Python-level checks before shell dispatch.
        # They harden the common case but cannot prevent escapes via absolute
        # paths, shell aliases, or native code.  Use a container backend for
        # true isolation.
        _workspace_terminal_prefix = ""
        try:
            from tools.workspace import get_workspace_manager
            _wsmgr = get_workspace_manager()

            # Trigger startup cleanup once per process (clears expired workspaces
            # from previous invocations without requiring a separate daemon).
            _wsmgr.cleanup_expired_once_at_startup()

            _ws = _wsmgr.get_for_task(effective_task_id)
            if _ws is None:
                # Lazily create workspace if a non-default policy is configured
                from tools.workspace import load_workspace_policy_from_config, FilesystemPolicy, WritePolicy
                _fp, _wp, _ttl, _roots = load_workspace_policy_from_config()
                if _fp != FilesystemPolicy.FULL_ACCESS or _wp != WritePolicy.FULL_WRITE:
                    _ws = _wsmgr.get_or_create_for_task(effective_task_id)

            if _ws is not None:
                # Check terminal command against workspace policy
                allowed, reason = _ws.check_terminal_command(command)
                if not allowed:
                    logger.warning(
                        "Terminal command denied by workspace policy: %s "
                        "(task=%s command=%.80s)",
                        reason, effective_task_id, command,
                    )
                    return json.dumps({
                        "output": "",
                        "exit_code": -1,
                        "error": f"Command denied by workspace policy: {reason}",
                        "status": "policy_denied",
                    }, ensure_ascii=False)

                # For workspace_only: inject cd into workspace as CWD prefix.
                # This ensures the shell starts inside the workspace directory.
                # NOTE: does not prevent 'cd /other/path' at runtime —
                # that requires OS-level confinement.
                _workspace_terminal_prefix = _ws.get_terminal_cwd_prefix()
                if _workspace_terminal_prefix:
                    logger.debug(
                        "workspace_only: prepending CWD prefix for task %s",
                        effective_task_id[:8],
                    )
        except Exception as _ws_exc:
            # Workspace policy errors must never silently allow a potentially
            # policy-violating command.  Re-raise for mutating operations;
            # log and proceed for reads only.
            logger.error(
                "Workspace policy check raised unexpected error (task=%s): %s",
                effective_task_id, _ws_exc,
            )
            # Conservative: deny on error
            return json.dumps({
                "output": "",
                "exit_code": -1,
                "error": f"Command denied: workspace policy check failed ({_ws_exc})",
                "status": "policy_error",
            }, ensure_ascii=False)
        ...
        # Apply workspace CWD prefix for local backend only.
        # Container backends (docker, modal, etc.) have their own workdir
        # handling; injecting cd into their shells is unnecessary and may
        # break their startup sequences.
        effective_command = command
        if _workspace_terminal_prefix and env_type == "local":
            effective_command = _workspace_terminal_prefix + command
