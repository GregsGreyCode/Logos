...
def _enforce_write_policy(path: str, task_id: str) -> Optional[str]:
    """
    Check workspace filesystem/write policy before a write operation.

    Returns a JSON error string if the write is denied, or None if allowed.

    This is a Python-level gate — not a kernel sandbox.  It resolves symlinks
    and normalises paths before checking so that ../escape and symlink tricks
    are caught.  See tools/workspace.py for the full trust-model commentary.
    """
    try:
        from tools.workspace import check_write_allowed
        allowed, reason = check_write_allowed(path, task_id)
        if not allowed:
            logger.warning("write_file denied by workspace policy: %s (path=%s task=%s)",
                           reason, path, task_id)
            return json.dumps({"error": f"Write denied by workspace policy: {reason}"},
                              ensure_ascii=False)
    except Exception as e:
        # Policy check failure must never silently allow a write.
        # Log and deny to be conservative.
        logger.error("Workspace policy check raised unexpected error: %s", e)
        return json.dumps(
            {"error": f"Write denied: workspace policy check failed ({e})"},
            ensure_ascii=False,
        )
    return None
...
def write_file_tool(path: str, content: str, task_id: str = "default") -> str:
    ...
    try:
        # Enforce workspace filesystem/write policy before dispatching.
        denied = _enforce_write_policy(path, task_id)
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    try:
        # Enforce workspace policy before any write.
        # For replace mode: path is explicit.
        # For patch mode: we extract the target path(s) from the patch header.
        if mode == "replace" and path:
            denied = _enforce_write_policy(path, task_id)
            if denied is not None:
                return denied
        elif mode == "patch" and patch:
            # Extract target paths from V4A patch headers (*** Update File: <path>)
            import re as _re
            for target_path in _re.findall(r'^\*\*\* (?:Update|Add) File:\s*(.+)$',
                                           patch, _re.MULTILINE):
                denied = _enforce_write_policy(target_path.strip(), task_id)
                if denied is not None:
                    return denied
