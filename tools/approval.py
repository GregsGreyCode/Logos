...
Also contains ``create_policy_approval_request()`` which persists approval
rows for MCP access requests (the one live ActionPolicy-adjacent enforcement
path; see ``tools/mcp_access_tool.py``).
...
#
# ``check_policy_for_tool()`` used to live here. It was a ~130-line
# dispatch gate that wrapped ``gateway/auth/policy.py:check_tool()``
# and did filesystem-path enforcement. It was never called at runtime:
# the only consumer was a re-export in ``logos/policy/enforcement.py``
# which nothing imported. Approval gating comes from
# ``check_all_command_guards()`` (hardcoded DANGEROUS_PATTERNS + Tirith)
# and MCP access gating goes through ``mcp_access_tool.py``. Workspace
# scoping is enforced inline in ``agents/hermes/agent.py``.
#
# Deleted on the README-audit cleanup pass. ``create_policy_approval_request``
# below is LIVE — ``mcp_access_tool.py`` calls it to persist MCP access
# approval rows.
# =========================================================================
