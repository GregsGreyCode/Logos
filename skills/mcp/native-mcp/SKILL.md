...
Add MCP servers to `~/.logos/config.yaml` under the `mcp_servers` key:
...
1. Reads `mcp_servers` from `~/.logos/config.yaml`
...
No `mcp_servers` key in `~/.logos/config.yaml`, or it's empty. Add at least one server.
