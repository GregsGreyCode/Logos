...
async def _handle_chat(request: web.Request) -> web.StreamResponse:
    ...
    if raw_attachments:
        import base64
        from gateway.media_cache import (
    ...
    try:
        ...
        if (worker_entry and worker_entry.healthy):
            ...
            try:
                # Dispatch via v2 HTTP-in-sandbox (hermes-as-server).
                # Phase 2 made this the only dispatch path: Plan A-Prime
                # (per-task sandbox_worker.py subprocess) is gone.
                # Sandboxes without hermes_server_setup surface a clear
                # error — resurrect (LOG-61/62) reinstates setup for any
                # sandbox that survives a gateway restart.
                from gateway.worker_registry_v2 import dispatch_task_v2
                logger.info(
                    "dispatch: v2 (hermes-as-server) for %s", target_worker,
                )
                worker_result = await dispatch_task_v2(
