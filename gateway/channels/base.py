...
class BasePlatformAdapter(ABC):
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars (LOGOS_* takes precedence; HERMES_* is a deprecated alias):
          LOGOS_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          LOGOS_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          LOGOS_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        import random

        mode = (
            os.getenv("LOGOS_HUMAN_DELAY_MODE")
            or os.getenv("HERMES_HUMAN_DELAY_MODE")
            or "off"
        ).lower()
        if mode == "off":
            return 0.0
        min_ms = int(
            os.getenv("LOGOS_HUMAN_DELAY_MIN_MS")
            or os.getenv("HERMES_HUMAN_DELAY_MIN_MS")
            or "800"
        )
        max_ms = int(
            os.getenv("LOGOS_HUMAN_DELAY_MAX_MS")
            or os.getenv("HERMES_HUMAN_DELAY_MAX_MS")
            or "2500"
        )
        if mode == "natural":
