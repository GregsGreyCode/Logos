...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Logos terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Logos' CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
Every installed skill in `~/.logos/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.logos/config.yaml`:
...
CLI sessions are stored in Hermes's SQLite state database under `~/.logos/state.db`. The database keeps:
