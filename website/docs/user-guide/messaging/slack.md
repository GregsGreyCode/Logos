...
sidebar_position: 4
title: "Slack"
description: "Set up Logos as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Logos to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
4. Enter an app name (e.g., "Logos") and select your workspace
5. Click **Create App**
...
```bash
logos gateway setup    # Select Slack when prompted
```
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Logos what time is it?`) |
...
Make sure the bot has been **invited to the channel** (`/invite @Logos`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Logos` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Logos` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Logos`)
...
- Tokens should be stored in `~/.logos/.env` (file permissions `600`)
