...
description: "Overview of Logos' tools — what's available, how toolsets work, and terminal backends"
---
...
# Configure
logos config set terminal.backend singularity
logos config set terminal.singularity_image ~/python.sif
```
...
modal setup
logos config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.logos/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.logos/.env`.
:::
