...
DEFAULT_CONFIG = {
    ...
    # Workspace isolation and filesystem policy.
    # Controls which paths the agent is allowed to write and how exec tools
    # behave.  Python-level enforcement only — not a kernel sandbox.
    # See tools/workspace.py for trust model and limitations.
    "workspace": {
        # filesystem_policy: controls which paths writes are allowed to.
        #   full_access    — no path restrictions (default, current behaviour)
        #   workspace_only — writes confined to the per-run workspace directory
        #   repo_scoped    — writes confined to paths listed in repo_roots
        #   read_only      — no writes permitted anywhere
        "filesystem_policy": "full_access",

        # write_policy: controls how writes are executed.
        #   full_write — execute writes normally (default)
        #   dry_run    — log writes but do not execute; mutating shell commands blocked
        #   read_only  — no writes at all (same effect as filesystem_policy=read_only)
        "write_policy": "full_write",

        # Base directory for per-run workspace directories.
        "workspace_base_dir": "~/.hermes/workspaces",

        # How long to keep workspace directories before automatic cleanup (hours).
        # Set to 0 to disable TTL (workspaces persist until manually cleaned).
        "workspace_ttl_hours": 24,

        # Allowed write roots for repo_scoped policy.
        # Each entry is an absolute path.  Symlinks are resolved before checking.
        # Example: ["/home/user/projects/myrepo", "/home/user/projects/otherrepo"]
        "repo_roots": [],
    },
