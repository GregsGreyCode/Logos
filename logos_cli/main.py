...
from logos_cli.config import get_env_path, get_logos_home
...
def _has_any_provider_configured() -> bool:
    ...
    from logos_cli.config import get_env_path, get_hermes_home
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from logos_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from logos_cli.config import get_env_value, save_env_value
    ...
    from logos_cli.models import model_ids
...
def _model_flow_custom(config):
    ...
    """
    from logos_cli.auth import _save_model_choice, deactivate_provider
    from logos_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from logos_cli.auth import _save_model_choice, deactivate_provider
    from logos_cli.config import save_env_value, load_config, save_config
    from logos_cli.models import fetch_api_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from logos_cli.config import save_anthropic_oauth_token
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from logos_cli.models import _PROVIDER_MODELS
...
def cmd_logout(args):
    ...
    from logos_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from logos_cli.status import show_status
    show_status(args)
...
def _update_via_zip(args):
    ...
    branch = "main"
    zip_url = f"https://github.com/GregsGreyCode/Logos/archive/refs/heads/{branch}.zip"
...
def cmd_update(args):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  git clone https://github.com/GregsGreyCode/Logos.git && cd Logos && pip install -e .")
            sys.exit(1)
...
"""
    ...
    def cmd_pairing(args):
        from logos_cli.pairing import pairing_command
    ...
    def cmd_claw(args):
        from logos_cli.claw import claw_command
        claw_command(args)
