...
def load_cli_config() -> Dict[str, Any]:
    ...
    # Normalize config key: the new config system (logos_cli/config.py) and all
...
try:
    from logos_cli.skin_engine import init_skin_from_config
...
from logos_cli.commands import COMMANDS, SlashCommandCompleter
from logos_cli import callbacks as _callbacks
...
from logos_cli.callbacks import prompt_for_secret
...
def _accent_hex() -> str:
    ...
    try:
        from logos_cli.skin_engine import get_active_skin
...
class HermesCLI:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if self._model_is_default:
            ...
            try:
                from logos_cli.codex_models import get_codex_model_ids
    ...
    def _init_agent(self) -> bool:
        ...
        try:
            ...
            self._runner = AgentRunner(HermesAdapter(self.agent))
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        img_dir = Path(os.getenv("LOGOS_HOME") or os.getenv("HERMES_HOME") or str(Path.home() / ".logos")) / "images"
    ...
    def _handle_paste_command(self):
        ...
        """
        from logos_cli.clipboard import has_clipboard_image
    ...
    def show_help(self):
        ...
        try:
            from logos_cli.skin_engine import get_active_help_header
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to logos_cli.skills_hub."""
        from logos_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        if cmd_lower in ("/quit", "/exit", "/q"):
            ...
            parts = cmd_original.split(maxsplit=1)
            if len(parts) > 1:
                from logos_cli.auth import resolve_provider
                from logos_cli.models import (
                    parse_model_input,
            ...
            self._show_usage()
        elif cmd_lower.startswith("/runs"):
            self._handle_runs_command(cmd_original)
        elif cmd_lower.startswith("/evals"):
            self._handle_evals_command(cmd_original)
        elif cmd_lower.startswith("/metrics"):
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from logos_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_runs_command(self, command: str = "/runs"):
        ...
        # Subcommands: replay, clone
        if len(parts) >= 3 and parts[1].lower() == "replay":
            self._runs_replay(db, parts[2])
            return
        if len(parts) >= 3 and parts[1].lower() == "clone":
            self._runs_clone(db, parts[2])
            return

        # /runs <id> — detail view
        if len(parts) == 2 and parts[1] not in ("--help", "help"):
            self._runs_detail(db, parts[1])
            return

        # /runs [--status <s>] [--source <s>] — list
        status_filter = None
        source_filter = None
        i = 1
        while i < len(parts):
            if parts[i] in ("--status", "-s") and i + 1 < len(parts):
                status_filter = parts[i + 1]
                i += 2
            elif parts[i] in ("--source", "-p") and i + 1 < len(parts):
                source_filter = parts[i + 1]
                i += 2
            else:
                i += 1

        self._runs_list(db, status=status_filter, source=source_filter)
    ...
    def _runs_list(self, db, status: str = None, source: str = None):
        ...
        if not runs:
            ...
            print(f"\n  No run records found{suffix}.")
            print("  Run records are created automatically when the agent executes.")
            return
        ...
        try:
            ...
            table.add_column("Model", width=20, no_wrap=True)
            ...
            for r in runs:
                st = r.get("status", "?")
                clr = _STATUS_CLR.get(st, "white")
                tools_count = len(r.get("tool_sequence") or [])
                model_short = (r.get("model") or "")
                if len(model_short) > 19:
                    model_short = "…" + model_short[-18:]
                prompt = (r.get("user_message_preview") or "")[:50]
                if len(r.get("user_message_preview") or "") > 50:
                    prompt += "…"
                table.add_row(
                    fmt_run_id_short(r.get("id", "")),
                    f"[{clr}]{fmt_status(st)}[/{clr}]",
            ...
            _console.print(
                f"  [dim]Showing {len(runs)} run(s). "
                "Use [bold]/runs <id>[/bold] for details · "
                "[bold]/runs replay <id>[/bold] to replay · "
                "[bold]/runs clone <id>[/bold] to clone[/dim]\n"
            )
        except ImportError:
            # Fallback plain text
            print()
            print(f"  {'ID':9}  {'Status':16}  {'Started':19}  {'Dur':7}  {'Src':8}  {'Tools':5}  Prompt")
            print("  " + "-" * 80)
            for r in runs:
                st = r.get("status", "?")
                tools_count = len(r.get("tool_sequence") or [])
                prompt = (r.get("user_message_preview") or "")[:40]
                print(
                    f"  {fmt_run_id_short(r.get('id','')):<9}  "
                    f"{fmt_status(st):<16}  "
                    f"{fmt_ts(r.get('started_at')):<19}  "
                    f"{fmt_duration(r):<7}  "
                    f"{(r.get('source') or '?'):<8}  "
                    f"{tools_count:<5}  {prompt}"
                )
            print()
    ...
    def _runs_detail(self, db, run_id_prefix: str):
        ...
        run = None
        if len(run_id_prefix) == 36:
            run = replayer.get_run(run_id_prefix)
        if run is None:
            # Try prefix match
            all_runs = replayer.list_runs(limit=200)
            matches = [r for r in all_runs if (r.get("id") or "").startswith(run_id_prefix)]
            if len(matches) == 1:
                run = matches[0]
            elif len(matches) > 1:
                print(f"\n  Ambiguous prefix '{run_id_prefix}' matches {len(matches)} runs:")
                for m in matches[:5]:
                    print(f"    {m['id']}  {fmt_ts(m.get('started_at'))}  {fmt_status(m.get('status','?'))}")
                return

        if run is None:
            print(f"\n  Run '{run_id_prefix}' not found.")
            return
        ...
        _STATUS_CLR = {
            ...
            "failed": "red", "interrupted": "yellow", "max_iterations": "dim",
        }
        clr = _STATUS_CLR.get(st, "white")

        _pr("")
        _pr(f"  [bold]Run {run.get('id', '?')}[/bold]" if _console else f"  Run {run.get('id', '?')}")
        _pr(f"  Status:   [{clr}]{fmt_status(st)}[/{clr}]" if _console else f"  Status:   {fmt_status(st)}")
        _pr(f"  Started:  {fmt_ts(run.get('started_at'))}")
        _pr(f"  Duration: {fmt_duration(run)}")
        _pr(f"  Source:   {run.get('source') or '—'}")
        _pr(f"  Model:    {run.get('model') or '—'}")
        _pr(f"  Provider: {run.get('provider') or '—'}")
        _pr(f"  Session:  {(run.get('session_id') or '—')[:36]}")
        _pr(f"  Trigger:  {run.get('trigger_type') or '—'}")
        if run.get("parent_run_id"):
            _pr(f"  Parent:   {run['parent_run_id'][:8]}…")
        _pr(f"  API calls:{run.get('api_call_count') or 0}")

        prompt = run.get("user_message_preview") or ""
        if prompt:
            _pr("")
            _pr(f"  [bold]Prompt[/bold]" if _console else "  Prompt")
            _pr(f"    {prompt}")
        ...
        if seq:
            _pr("")
            _pr(f"  [bold]Tool Timeline ({len(seq)} calls)[/bold]" if _console else f"  Tool Timeline ({len(seq)} calls)")
            if _console:
                from rich.table import Table
                tbl = Table(show_header=True, header_style="dim", show_lines=False, padding=(0, 1), box=box.SIMPLE)
                tbl.add_column("#", width=3, style="dim")
                tbl.add_column("Tool", width=20)
                tbl.add_column("OK", width=4)
                tbl.add_column("ms", width=7)
                tbl.add_column("Args preview")
                for idx, entry in enumerate(seq, 1):
                    ok = entry.get("ok")
                    ok_str = "[green]✓[/green]" if ok is True else ("[red]✗[/red]" if ok is False else "?")
                    ms = f"{entry.get('ms', 0):.0f}" if entry.get("ms") is not None else "—"
                    err = entry.get("err", "")
                    args = entry.get("args", "")
                    preview = f"[red]{err[:60]}[/red]" if err else args[:60]
                    tbl.add_row(str(idx), entry.get("tool", "?"), ok_str, ms, preview)
                _console.print("  ", end="")
                _console.print(tbl)
            else:
                for idx, entry in enumerate(seq, 1):
                    ok = "✓" if entry.get("ok") else "✗"
                    ms = f"{entry.get('ms', 0):.0f}ms" if entry.get("ms") is not None else "?"
                    print(f"    {idx:2}. [{ok}] {entry.get('tool','?'):<22} {ms:<8} {entry.get('args','')[:50]}")

        # Approval events
        approvals = run.get("approval_events") or []
        if approvals:
            _pr("")
            _pr(f"  [bold]Approval Events ({len(approvals)})[/bold]" if _console else f"  Approval Events ({len(approvals)})")
            for ev in approvals:
                icon = "✅" if ev.get("approved") else "❌"
                _pr(f"    {icon} [{ev.get('type','?')}] {ev.get('cmd','')[:60]}")

        # Final output
        out = run.get("final_output_preview") or ""
        if out:
            _pr("")
            _pr(f"  [bold]Final Output (preview)[/bold]" if _console else "  Final Output (preview)")
            for line in out[:300].splitlines()[:8]:
                _pr(f"    {line}")
            if len(out) > 300:
                _pr("    …")
        ...
        if run.get("error_details"):
            _pr("")
            _pr(f"  [bold red]Error[/bold red]" if _console else "  Error")
            _pr(f"    {run['error_details'][:200]}")

        _pr("")
        short_id = fmt_run_id_short(run.get("id", ""))
        _pr(f"  [dim]/runs replay {short_id}  — re-run this prompt[/dim]" if _console
            else f"  /runs replay {short_id}  — re-run this prompt")
        _pr(f"  [dim]/runs clone {short_id}   — start new session with same prompt[/dim]\n" if _console
            else f"  /runs clone {short_id}   — start new session with same prompt\n")
    ...
    def _runs_replay(self, db, run_id_prefix: str):
        ...
        is_destructive = replayer.has_destructive_tools(run)
        tool_summary = replayer.tool_summary(run)

        prompt = run.get("user_message_preview") or ""
        if not prompt.strip():
            print(f"\n  Run {run_id_prefix[:8]} has no stored prompt — cannot replay.")
            return

        print(f"\n  Replay run {run.get('id','?')[:8]}…")
        print(f"  Prompt: {prompt[:80]}")
        if tool_summary != "(none)":
            print(f"  Tools used: {tool_summary}")

        if is_destructive:
            print()
            print("  ⚠️  This run used tools that can modify files or run commands.")
            print("      Any dangerous commands will require re-approval before executing.")
            ans = input("  Proceed with replay? [y/N] ").strip().lower()
            if ans not in ("y", "yes"):
                print("  Replay cancelled.")
                return

        # Re-submit the prompt via the normal input queue so it goes through
        # the standard agent execution path (including approval prompts).
        print()
        print(f"  Submitting: {prompt[:60]}…\n")
        if hasattr(self, '_pending_input'):
            self._pending_input.put(prompt)
        else:
            print("  (paste the prompt above to execute manually)")
    ...
    def _runs_clone(self, db, run_id_prefix: str):
        ...
        prompt = run.get("user_message_preview") or ""
        if not prompt.strip():
            print(f"\n  Run {run_id_prefix[:8]} has no stored prompt — cannot clone.")
            return

        # Start a new session and queue the prompt for execution
        self.new_session(silent=True)
        print(f"\n  ✨ New session. Submitting cloned prompt from run {run_id_prefix[:8]}:")
        print(f"  {prompt[:80]}\n")
        if hasattr(self, '_pending_input'):
            self._pending_input.put(prompt)
        else:
    ...
    def _resolve_run(self, replayer, run_id_prefix: str):
        """Resolve a run ID prefix to a full run record."""
        if len(run_id_prefix) == 36:
            return replayer.get_run(run_id_prefix)
        all_runs = replayer.list_runs(limit=200)
        matches = [r for r in all_runs if (r.get("id") or "").startswith(run_id_prefix)]
        if len(matches) == 1:
            return matches[0]
    ...
    def _handle_evals_command(self, command: str = "/evals"):
        """Handle /evals [list | run <suite> | results [suite] | suites] command."""
        parts = command.strip().split()
        ...
        try:
            ...
            print(f"  Eval framework not available: {exc}")
            return
        ...
        if sub in ("list", "suites", "help"):
            self._evals_list(list_suites())
            return

        if sub == "results":
            suite_name = parts[2] if len(parts) >= 3 else None
            self._evals_results(db, suite_name)
            return

        if sub == "run":
            suite_name = parts[2] if len(parts) >= 3 else None
            if not suite_name:
                print("  Usage: /evals run <suite_name>")
                print("  Use /evals list to see available suites.")
                return
            suite = get_suite(suite_name)
            if suite is None:
                print(f"  Eval suite {suite_name!r} not found.")
                print("  Use /evals list to see available suites.")
                return
            self._evals_run(suite, db)
            return

        print(f"  Unknown evals subcommand: {sub!r}")
        print("  Usage: /evals [list | run <suite> | results [suite]]")

    def _evals_list(self, suites):
        """Display available eval suites."""
        try:
            from rich.console import Console
            from rich.table import Table
            _console = Console()
            ...
            table.add_column("Cases", width=6)
            table.add_column("Description")
            for s in suites:
                table.add_row(s.name, str(len(s.cases)), s.description[:70])
            _console.print()
            _console.print(table)
            _console.print(
                "  [dim]Run with [bold]/evals run <name>[/bold]  "
                "View results with [bold]/evals results <name>[/bold][/dim]\n"
            )
        except ImportError:
            print("\n  Available eval suites:")
            for s in suites:
                print(f"    {s.name:<30} ({len(s.cases)} cases) — {s.description[:60]}")
            print()

    def _evals_run(self, suite, db):
        """Execute an eval suite and display results."""
        if not self._init_agent():
            print("  Cannot run evals: agent credentials not available.")
            return

        if self.agent is None:
            print("  Cannot run evals: agent not initialized.")
            return

        print(f"\n  Running eval suite: {suite.name!r} ({len(suite.cases)} cases)\n")

        try:
            from evals.runner import run_eval_suite
            suite_result = run_eval_suite(suite, self.agent, db=db)
        except Exception as exc:
            print(f"  Eval suite failed: {exc}")
            return
        ...
        try:
            ...
            table = Table(
                title=f"Eval Results: {suite.name}",
                title_style="bold",
                show_header=True,
                header_style="bold dim",
                padding=(0, 1),
            ...
            table.add_column("Pass/Fail", width=10)
            table.add_column("Score", width=7)
            table.add_column("Dur", width=7)
            table.add_column("Details")

            _STATUS = {True: "[green]✅ PASS[/green]", False: "[red]❌ FAIL[/red]"}

            for cr in suite_result.case_results:
                dur = f"{cr.duration_seconds:.1f}s"
                details_parts = []
                for ar in cr.assertion_results:
                    icon = "✓" if ar.passed else "✗"
                    details_parts.append(f"{icon} {ar.assertion_type}: {ar.reason[:40]}")
                details = "\n".join(details_parts) if details_parts else (cr.error or "—")
                table.add_row(
                    cr.case_name[:35],
                    _STATUS[cr.passed],
                    f"{cr.score:.0%}",
            ...
            _console.print()
            _console.print(table)
            pass_rate = f"{suite_result.pass_rate:.0%}"
            color = "green" if suite_result.pass_rate == 1.0 else ("yellow" if suite_result.pass_rate > 0.5 else "red")
            _console.print(
                f"\n  [{color}]Suite: {suite_result.passed_cases}/{suite_result.total_cases} passed "
                f"({pass_rate}) in {suite_result.duration_seconds:.1f}s[/{color}]\n"
            )
        except ImportError:
            print(suite_result.summary())

    def _evals_results(self, db, suite_name: str = None):
        """Display recent eval results from the DB."""
        if db is None:
            print("  No database available.")
            return
        try:
            results = db.list_eval_results(suite_name=suite_name, limit=30)
        except Exception as exc:
            print(f"  Cannot read eval results: {exc}")
            return

        if not results:
            suffix = f" for suite {suite_name!r}" if suite_name else ""
            print(f"\n  No eval results found{suffix}.")
            print("  Run /evals run <suite> to execute a suite.\n")
            return
        ...
        try:
            ...
            _console = Console()
            table = Table(
                title=f"Eval Results{' — ' + suite_name if suite_name else ''}",
                title_style="bold",
                show_header=True,
                header_style="bold dim",
                padding=(0, 1),
            ...
            table.add_column("P/F", width=5)
            table.add_column("Score", width=6)
            table.add_column("Run at", style="dim", width=16)
            ...
            for r in results:
                ...
                table.add_row(
                    (r.get("suite_name") or "")[:20],
                    (r.get("case_name") or "")[:28],
                    pf,
                    score,
                    ts_str,
                    (r.get("error") or "")[:50],
            ...
            for r in results:
                ...
                print(f"  [{pf}] {r.get('suite_name')} / {r.get('case_name')}  score={r.get('score', 0):.0%}")

    # =========================================================================
    # /metrics — observability dashboard
    # =========================================================================

    def _handle_metrics_command(self, command: str = "/metrics"):
        """Handle /metrics [--days N] [prometheus] command."""
        parts = command.strip().split()
        ...
        i = 1
        while i < len(parts):
            if parts[i] in ("--days", "-d") and i + 1 < len(parts):
                try:
                    days = int(parts[i + 1])
                except ValueError:
                    pass
                i += 2
            elif parts[i] == "prometheus":
                prom_mode = True
                i += 1
            else:
                i += 1
        ...
        if prom_mode:
            print(engine.export_prometheus(days=days))
        else:
            print(engine.format_dashboard(days=days))
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        try:
            ...
            def run_agent():
                ...
                result = self._runner.run(AgentContext(
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from logos_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from logos_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from logos_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    if query:
        if quiet:
            ...
            if cli._init_agent():
                ...
                result = cli._runner.run(AgentContext(user_message=query)).to_dict()
