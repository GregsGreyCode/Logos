...
The canonical source is `souls/default/soul.md`. This module reads from it
so there is a single source of truth — edit that file, not this one.

Falls back to the hardcoded string only when the file cannot be found (e.g.
installed as a non-editable wheel without the repo alongside it).
"""
...
def _load_soul() -> str:
    soul_file = Path(__file__).parent.parent / "souls" / "default" / "soul.md"
    if soul_file.exists():
        return soul_file.read_text(encoding="utf-8")
    return _FALLBACK_SOUL_MD


# Fallback used only when souls/default/soul.md is not present on disk (e.g.
# pip install from a tarball rather than an editable clone). Keep in sync with
# souls/default/soul.md manually.
# TODO: Replace with importlib.resources if Logos is ever packaged as a
#       non-editable distribution that needs to ship souls/ inside the wheel.
