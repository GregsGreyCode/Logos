...
**No native desktop app — current install requires Docker or Python.**
The onboarding wizard assumes the model server (Ollama, LM Studio) is reachable at a known network address. For a technically confident homelab user that is fine, but it is a hard barrier for anyone less comfortable with networking. The right long-term answer for broader adoption is a native desktop app — an Electron or Go/Wails binary that packages the Logos gateway, bundles the onboarding UI, and can detect local model servers automatically without the user knowing what a port is. On Windows specifically, the current path (Docker Desktop or Python venv + `pip install`) is a significant UX cliff. A signed `.exe` that runs `logos.exe` and opens the browser to the setup wizard is the correct Windows story. Go is the right implementation language for this: single binary, no runtime, cross-platform, small footprint. Until then, Docker Compose is the recommended install path, which excludes a meaningful share of potential users.
