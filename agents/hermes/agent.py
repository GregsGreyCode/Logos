...
from core.model_tools import get_tool_definitions, handle_function_call, check_toolset_requirements
...
class AIAgent:
    ...
    def __init__(
        ...
        action_policy=None,   # ActionPolicy | None — enforced in _invoke_tool
        auth_user_id: str = None,  # auth DB user id (usr_...) for approval requests
        workspace_path: str = None,   # per-run isolated workspace path
    ):
        """
        ...
        if self.api_mode == "anthropic_messages":
            ...
            # Inject X-Provider-Policy when an action policy restricts provider trust.
            # The ai-router reads this header and enforces it server-side as defence-in-depth.
            if action_policy is not None:
                from gateway.auth.policy import ProviderPolicy as _PP
                _pp = getattr(action_policy, "provider_policy", _PP.ANY)
                if _pp != _PP.ANY:
                    _hdrs = client_kwargs.setdefault("default_headers", {})
                    _hdrs["X-Provider-Policy"] = _pp
        ...
        if not skip_memory:
            try:
                ...
                if self._memory_enabled or self._user_profile_enabled:
                    ...
                    _shared_dir_str = mem_config.get("shared_memory_dir", "")
                    _shared_dir = _Path(_shared_dir_str) if _shared_dir_str else None
                    _shared_write_str = mem_config.get("shared_write_dir", "")
                    _shared_write = _Path(_shared_write_str) if _shared_write_str else None
                    self._memory_store = MemoryStore(
                        memory_char_limit=mem_config.get("memory_char_limit", 2200),
                        user_char_limit=mem_config.get("user_char_limit", 1375),
                        shared_memory_dir=_shared_dir,
                        shared_write_dir=_shared_write,
                    )
        ...
        try:
            from logos_cli.config import load_config as _load_skills_config
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if self._memory_store:
            ...
            # Shared memory from primary instance (read-only)
            shared_mem_block = self._memory_store.format_shared_for_system_prompt("memory")
            if shared_mem_block:
                prompt_parts.append(shared_mem_block)
            shared_user_block = self._memory_store.format_shared_for_system_prompt("user")
            if shared_user_block:
                prompt_parts.append(shared_user_block)
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from logos_cli.auth import resolve_codex_runtime_credentials
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        if not reasoning_text:
            ...
            if think_blocks:
                ...
                reasoning_from_think_block = bool(reasoning_text)
        ...
        msg = {
            ...
            # Track source so the API-prep loop knows NOT to re-inject as reasoning_content
            # for providers that use <think> blocks (e.g. LM Studio / qwen local models).
            # When True: reasoning came from parsing content, not a native API field.
            "reasoning_from_think_block": reasoning_from_think_block,
        }
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict) -> dict:
        ...
        if not isinstance(tool_calls, list):
            return api_msg
        # Strip all non-OpenAI-spec fields from tool_call dicts.
        # call_id / response_item_id are Codex Responses API internals.
        # extra_content is a Gemini-specific field.
        # Keeping only id, type, function keeps the payload spec-compliant
        # for LM Studio and any other strict OpenAI-compatible provider.
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str) -> str:
        ...
        Policy enforcement is delegated to ToolRegistry.invoke() (Logos platform
        layer) so the gate runs consistently for every non-agent-level tool call.
        """

        # ── Terminal workspace scoping ────────────────────────────────────
        # For exec tools, scope the working directory to the active workspace
        # (workspace_only policy) or a repo root (repo_scoped policy).
        # This ensures shell sessions start inside the allowed boundary and
        # relative paths resolve within it.
        #
        # LIMITATION: This injects workdir for each tool call, but cannot
        # prevent the shell from `cd`-ing away once it starts.  True cwd
        # confinement requires a persistent per-session shell with enforcement
        # at the pty/kernel level.
        if (
            ...
            try:
                from gateway.auth.policy import (
                    categorise_tool as _cat_tool,
                    FilesystemPolicy,
                    ACTION_EXEC,
                )
                if _cat_tool(function_name) == ACTION_EXEC:
                    fs_policy = getattr(self._action_policy, "filesystem_policy", None)

                    if fs_policy == FilesystemPolicy.WORKSPACE_ONLY:
                        provided_workdir = function_args.get("workdir")
                        if not provided_workdir:
                            # Inject workspace as default cwd
                            function_args = dict(function_args)
                            function_args["workdir"] = str(self._workspace_path)
                            logger.debug(
                                "terminal_scope: injected workspace workdir=%s tool=%s",
                                self._workspace_path, function_name,
                            )
                        else:
                            from gateway.workspace import is_within_workspace as _iw
                            if not _iw(provided_workdir, self._workspace_path):
                                # Override: workdir is outside workspace — clamp to workspace
                                logger.warning(
                                    "terminal_scope: workdir %r is outside workspace %s "
                                    "— overriding to workspace root (tool=%s)",
                                    provided_workdir, self._workspace_path, function_name,
                                )
                                function_args = dict(function_args)
                                function_args["workdir"] = str(self._workspace_path)

                    elif fs_policy == FilesystemPolicy.REPO_SCOPED:
                        provided_workdir = function_args.get("workdir")
                        if provided_workdir:
                            from gateway.auth.policy import _get_repo_roots
                            import os as _os
                            resolved_wd = _os.path.realpath(
                                _os.path.expanduser(provided_workdir)
                            )
                            repo_roots = _get_repo_roots()
                            sep = _os.sep
                            # Also allow workspace itself
                            ws_real = _os.path.realpath(str(self._workspace_path))
                            in_ws = resolved_wd == ws_real or resolved_wd.startswith(ws_real + sep)
                            in_repo = any(
                                resolved_wd == r or resolved_wd.startswith(r + sep)
                                for r in repo_roots
                            )
                            if not in_ws and not in_repo:
                                logger.warning(
                                    "terminal_scope: repo_scoped workdir %r outside allowed "
                                    "roots — clamping to workspace (tool=%s)",
                                    provided_workdir, function_name,
                                )
                                function_args = dict(function_args)
                                function_args["workdir"] = str(self._workspace_path)
            except Exception as _scope_err:
                logger.debug("terminal_scope error for %s: %s", function_name, _scope_err)
        # ─────────────────────────────────────────────────────────────────
    ...
    def run_conversation(
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            for idx, msg in enumerate(messages):
                ...
                # For assistant messages, pass reasoning back as reasoning_content
                # ONLY for providers that natively support the field (Moonshot AI,
                # Novita, some OpenRouter models).  When reasoning was extracted from
                # <think> blocks (e.g. LM Studio / local qwen models), do NOT inject
                # it as reasoning_content — those providers don't accept that field and
                # will return 422 / close the connection, breaking the tool-call loop.
                if msg.get("role") == "assistant":
                ...
                # Remove internal-only fields before sending to any provider
                for _drop_key in ("reasoning", "reasoning_from_think_block"):
                    if _drop_key in api_msg:
                        api_msg.pop(_drop_key)
                # Remove finish_reason - not accepted by strict APIs (e.g. Mistral)
                if "finish_reason" in api_msg:
                    api_msg.pop("finish_reason")
                # Strip Codex Responses API fields (call_id, response_item_id,
                # extra_content) for ALL chat_completions providers.  These fields
                # are Hermes-internal / Codex-only and are rejected as unknown by
                # LM Studio, Mistral, and other OpenAI-compatible backends.
                # Codex Responses API uses a separate api_mode so it is unaffected.
            ...
            # Log the roles/types of the last few messages to aid tool-call debugging
            _tail_roles = [
                f"{m.get('role')}{'[tc]' if m.get('tool_calls') else ''}{'[tr]' if m.get('tool_call_id') else ''}"
                for m in api_messages[-4:]
            ]
            logger.debug(
                "API call #%d: %d msgs ~%d tok tail=%s",
                api_call_count, len(api_messages), approx_tokens, _tail_roles,
            )
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    tc_names = [tc.function.name for tc in assistant_message.tool_calls]
                    logger.info(
                        "Tool-call turn #%d: finish_reason=%r tools=%r content_len=%d",
                        api_call_count,
                        finish_reason,
                        tc_names,
