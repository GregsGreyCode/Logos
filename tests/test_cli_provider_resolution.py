...
from logos_cli import main as hermes_main
...
def _import_cli():
    ...
    return importlib.import_module("logos_cli.cli")


def test_logos_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}
    ...
    monkeypatch.setattr("logos_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("logos_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("logos_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
