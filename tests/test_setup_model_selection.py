...
class TestSetupProviderModelSelection:
    ...
    @patch("logos_cli.models.fetch_api_models", return_value=[])
    @patch("logos_cli.config.get_env_value", return_value="fake-key")
    def test_falls_back_to_default_models_without_crashing(
        ...
        from logos_cli.setup import _setup_provider_model_selection
        ...
        with patch("logos_cli.auth.PROVIDER_REGISTRY", mock_provider_registry):
    ...
    def test_custom_model_selection(
        ...
        from logos_cli.setup import _setup_provider_model_selection, _DEFAULT_PROVIDER_MODELS
