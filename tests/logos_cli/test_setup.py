...
from logos_cli.auth import _update_config_for_provider, get_active_provider
...
def test_nous_oauth_setup_keeps_current_model_when_syncing_disk_provider(
    ...
    monkeypatch.setattr("logos_cli.auth._login_nous", _fake_login_nous)
    monkeypatch.setattr(
        "logos_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "logos_cli.auth.fetch_nous_models",
...
def test_custom_setup_clears_active_oauth_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("logos_cli.setup.prompt_choice", lambda *args, **kwargs: 3)
