"""Tests for provider-aware `/model` validation in logos_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("logos_cli.models.fetch_api_models", return_value=api_models):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("logos_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
