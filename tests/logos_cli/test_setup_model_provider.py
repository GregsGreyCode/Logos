...
from logos_cli.config import load_config, save_config, save_env_value
from logos_cli.setup import setup_model_provider
...
def test_setup_keep_current_custom_from_config_does_not_fall_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("logos_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("logos_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("logos_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("logos_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("logos_cli.auth.detect_external_credentials", lambda: [])
...
def test_setup_keep_current_config_provider_uses_provider_specific_model_menu(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("logos_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("logos_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("logos_cli.models.provider_model_ids", lambda provider: [])
...
def test_setup_switch_custom_to_codex_clears_custom_endpoint_and_updates_config(tmp_path, monkeypatch):
    ...
    picks = iter([1, 0])
    monkeypatch.setattr("logos_cli.setup.prompt_choice", lambda *args, **kwargs: next(picks))
    monkeypatch.setattr("logos_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("logos_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("logos_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("logos_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("logos_cli.auth._login_openai_codex", lambda *args, **kwargs: None)
    monkeypatch.setattr(
