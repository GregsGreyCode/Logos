...
class TestVprintForceParameter:
    ...
    def test_error_messages_use_force_in_run_agent(self):
        """Verify that critical error _vprint calls in agents/hermes/agent.py
        include force=True."""
        with open("agents/hermes/agent.py", "r") as f:
            source = f.read()
...
class TestCtrlCResetsContinuousMode:
    ...
    def test_ctrl_c_handler_resets_voice_continuous(self):
        ...
        with open("logos_cli/cli.py") as f:
            source = f.read()
...
class TestEnableVoiceModeReal:
    ...
    @patch("logos_cli.cli._cprint")
    @patch("logos_cli.config.load_config", return_value={"voice": {}})
    ...
    @patch("logos_cli.cli._cprint")
    @patch("logos_cli.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
    ...
    @patch("logos_cli.cli._cprint")
    @patch("logos_cli.config.load_config", side_effect=Exception("broken config"))
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
...
class TestVoiceSpeakResponseReal:
    ...
    @patch("logos_cli.cli._cprint")
    @patch("logos_cli.cli.os.unlink")
    @patch("logos_cli.cli.os.path.getsize", return_value=1000)
    @patch("logos_cli.cli.os.path.isfile", return_value=True)
    @patch("logos_cli.cli.os.makedirs")
...
class TestVoiceStopAndTranscribeReal:
    ...
    @patch("logos_cli.cli.os.unlink")
    @patch("logos_cli.cli.os.path.isfile", return_value=True)
    @patch("logos_cli.config.load_config", return_value={"stt": {}})
    ...
    @patch("logos_cli.cli.os.unlink")
    @patch("logos_cli.cli.os.path.isfile", return_value=True)
    @patch("logos_cli.config.load_config", return_value={"stt": {"model": "whisper-large-v3"}})
    @patch("tools.voice_mode.transcribe_recording",
